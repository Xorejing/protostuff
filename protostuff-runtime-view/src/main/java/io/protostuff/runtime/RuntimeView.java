...
public final class RuntimeView {
	 ...
	 * Returns a new view schema based from an existing one.
	 */
	public static <T> Schema<T> createFrom(RuntimeSchema<T> ms, Instantiator<T> instantiator, Factory vf,
			Predicate.Factory pf, String... args) {
	...
	public enum Factories implements Factory {
		...
		EXCLUDE_OPTIMIZED_FOR_MERGE_ONLY {
			@Override
			public <T> Schema<T> create(RuntimeSchema<T> ms, Instantiator<T> instantiator, Predicate.Factory pf,
					String[] args) {
		...
		INCLUDE {
			...
			public <T> Schema<T> create(final RuntimeSchema<T> ms, Instantiator<T> instantiator,
					Predicate.Factory factory, String[] args) {
				final HashMap<String, Field<T>> fieldsByName = new HashMap<String, Field<T>>();

				// int maxFieldNumber =
				includeAndAddTo(fieldsByName, ms.typeClass(), ms.getFields(), args);
	...
	static <T> HashMap<String, Field<T>> copyAndExclude(Class<? super T> typeClass, List<Field<T>> fields,
	...
	static <T> int includeAndAddTo(Map<String, Field<T>> map, Class<? super T> typeClass, List<Field<T>> fields,
			final String[] args) {
		if (args == null || args.length == 0)
