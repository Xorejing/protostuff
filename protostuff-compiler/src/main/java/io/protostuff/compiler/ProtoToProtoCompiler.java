...
public class ProtoToProtoCompiler extends STCodeGenerator {
	...
	public static String injectAfterAnnotation(Message extend, Message by, String extendProto, String byContent) {
		...
		if (matcher.find()) {
			...
			extendProto = extendProto.substring(0, openBracketIndex) + LINE_SEPARATOR + indentation + "// "
					+ generateTimestamp(extend, by) + LINE_SEPARATOR + insertIndentation(byContent, indentation)
	...
	public static String injectAfterOption(Message extend, Message by, String extendProto, String byContent) {
		Pattern messageRegexp = Pattern.compile(
				"([\\n\\r]?[ \\t]*message\\s+" + extend.getName()
						+ "\\s+\\{[^{}]*[\\n\\r][ \\t]*)(option\\s+extends\\s+=\\s+" + by.getName() + "\\s*;)",
				Pattern.MULTILINE);
		...
		if (matcher.find()) {
			...
			matcher.appendReplacement(sb, "$1" + "// " + generateTimestamp(extend, by) + LINE_SEPARATOR + indentation
					+ "// $2" + LINE_SEPARATOR + insertIndentation(byContent, indentation));
	...
	public static String generateIndentation(String indentation, int length) {
		StringBuilder builder = new StringBuilder(indentation == null ? "" : indentation);
