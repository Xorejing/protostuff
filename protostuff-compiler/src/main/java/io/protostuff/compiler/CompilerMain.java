...
public final class CompilerMain {
	...
	public static final boolean SILENT_MODE = Boolean
			.parseBoolean(System.getProperty("protostuff.compiler.silent_mode", "true"));

	public static final Pattern COMMA = Pattern.compile(",");

	static final HashMap<String, ProtoCompiler> __compilers = new HashMap<String, ProtoCompiler>();
	...
	public static List<ProtoModule> loadModules(Properties props, File baseDirForSource, File baseDirForOutput) {
		...
		for (String m : COMMA.split(moduleString)) {
			modules.add(
					loadModule(props, m.trim(), loader, baseDirForSource, baseDirForOutput, globalOptions, null, null));
		}

		return modules;
	}

	public static ProtoModule loadModule(Properties props, String name, CachingProtoLoader loader,
			File baseDirForSource, File baseDirForOutput, Properties globalOptions, String[] profileOptions,
			String[] rootProfileOptions) {
	...
	static ProtoModule newProtoModule(String source, String output, String encoding, String outputDir,
			File baseDirForSource, File baseDirForOutput) {
		if (baseDirForSource != null) {
			if (baseDirForOutput != null) {
				return new ProtoModule(new File(baseDirForSource, source), output, encoding,
						new File(baseDirForOutput, outputDir));
			}
	...
	public static void compile(ProtoModule module) throws Exception {
		...
		try {
			for (String output : COMMA.split(originalOutput)) {
				...
				if (!SILENT_MODE) {
					StringBuilder buffer = new StringBuilder().append("Successfully compiled proto from ")
							.append(module.getSource()).append(" to output: ").append(output);
	...
	static void compileProfile(Properties props, String profile, CachingProtoLoader loader, Properties globalOptions,
			String[] rootProfileOptions, final int nestCount) throws Exception {
		...
		for (String m : COMMA.split(moduleString)) {
			...
			if (m.charAt(0) == '@') {
				...
				compileProfile(props, m, loader, globalOptions, nestCount == 0 ? profileOptions : rootProfileOptions,
						nestCount + 1);
				continue;
			}

			compile(loadModule(props, m, loader, null, null, globalOptions, profileOptions,
