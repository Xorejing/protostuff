...
public abstract class IdStrategy {
	...
	public static final int ENUMS_BY_NAME = 1, AUTO_LOAD_POLYMORPHIC_CLASSES = 1 << 1,
			ALLOW_NULL_ARRAY_ELEMENT = 1 << 2, MORPH_NON_FINAL_POJOS = 1 << 3, MORPH_COLLECTION_INTERFACES = 1 << 4,
			MORPH_MAP_INTERFACES = 1 << 5, COLLECTION_SCHEMA_ON_REPEATED_FIELDS = 1 << 6,
			POJO_SCHEMA_ON_COLLECTION_FIELDS = 1 << 7, POJO_SCHEMA_ON_MAP_FIELDS = 1 << 8, DEFAULT_FLAGS;
	...
	protected IdStrategy(int flags, IdStrategy primaryGroup, int groupId) {
		if (primaryGroup != null) {
			...
			throw new RuntimeException(
					"An IdStrategy without a primaryGroup " + "(standalone) must have a groupId of zero.");
		}
	...
	protected <T> Schema<T> newSchema(Class<T> typeClass) {
		...
		/*
		 * if (fields.size() == 0) { throw new RuntimeException("All fields were excluded for " + rs.messageFullName() +
		 * " on group " + groupId); }
		 */

		return fields.size() == rs.getFieldCount() ? rs : new RuntimeSchema<T>(typeClass, fields, rs.instantiator);
	}
	...
	protected abstract <T> HasSchema<T> tryWritePojoIdTo(Output output, int fieldNumber, Class<T> clazz,
			boolean registered) throws IOException;

	protected abstract <T> HasSchema<T> writePojoIdTo(Output output, int fieldNumber, Class<T> clazz)
			throws IOException;
	...
	protected abstract <T> Schema<T> writeMessageIdTo(Output output, int fieldNumber, Message<T> message)
			throws IOException;
	 ...
	 */
	protected abstract <T> HasDelegate<T> tryWriteDelegateIdTo(Output output, int fieldNumber, Class<T> clazz)
			throws IOException;

	protected abstract <T> HasDelegate<T> transferDelegateId(Input input, Output output, int fieldNumber)
			throws IOException;
	...
	final DerivativeSchema POLYMORPHIC_POJO_ELEMENT_SCHEMA = new DerivativeSchema(this) {
		@Override
	...
	final PolymorphicEnumSchema POLYMORPHIC_ENUM_ELEMENT_SCHEMA = new PolymorphicEnumSchema(this) {
		@Override
		@SuppressWarnings("unchecked")
		protected void setValue(Object value, Object owner) {
			if (MapWrapper.class == owner.getClass())
	...
	final PolymorphicThrowableSchema POLYMORPHIC_THROWABLE_ELEMENT_SCHEMA = new PolymorphicThrowableSchema(this) {
		@Override
		@SuppressWarnings("unchecked")
		protected void setValue(Object value, Object owner) {
			if (MapWrapper.class == owner.getClass())
	...
	final Pipe.Schema<Object> DYNAMIC_VALUE_PIPE_SCHEMA = new Pipe.Schema<Object>(DYNAMIC_VALUE_SCHEMA) {
		@Override
	...
	final Pipe.Schema<Object> ARRAY_PIPE_SCHEMA = new Pipe.Schema<Object>(ARRAY_SCHEMA) {
		@Override
		protected void transfer(Pipe pipe, Input input, Output output) throws IOException {
			for (int number = input.readFieldNumber(wrappedSchema);; number = input.readFieldNumber(wrappedSchema)) {
				switch (number) {
				...
				case 1:
					...
					if (entry != input.mergeObject(entry, ENTRY_SCHEMA)) {
						...
						throw new IllegalStateException(
								"A Map.Entry will always be " + "unique, hence it cannot be a reference "
	...
	final Pipe.Schema<Map<Object, Object>> MAP_PIPE_SCHEMA = new Pipe.Schema<Map<Object, Object>>(MAP_SCHEMA) {
		@Override
		protected void transfer(Pipe pipe, Input input, Output output) throws IOException {
			for (int number = input.readFieldNumber(wrappedSchema);; number = input.readFieldNumber(wrappedSchema)) {
				switch (number) {
	...
	final Pipe.Schema<Entry<Object, Object>> ENTRY_PIPE_SCHEMA = new Pipe.Schema<Entry<Object, Object>>(ENTRY_SCHEMA) {
		@Override
		protected void transfer(Pipe pipe, Input input, Output output) throws IOException {
			for (int number = input.readFieldNumber(wrappedSchema);; number = input.readFieldNumber(wrappedSchema)) {
				switch (number) {
	...
	final Pipe.Schema<Object> OBJECT_PIPE_SCHEMA = new Pipe.Schema<Object>(OBJECT_SCHEMA) {
		@Override
		protected void transfer(Pipe pipe, Input input, Output output) throws IOException {
			ObjectSchema.transferObject(this, pipe, input, output, IdStrategy.this);
		}
	...
	final Pipe.Schema<Object> CLASS_PIPE_SCHEMA = new Pipe.Schema<Object>(CLASS_SCHEMA) {
		@Override
		protected void transfer(Pipe pipe, Input input, Output output) throws IOException {
	...
	final Pipe.Schema<Object> POLYMORPHIC_MAP_PIPE_SCHEMA = new Pipe.Schema<Object>(POLYMORPHIC_MAP_SCHEMA) {
		@Override
		protected void transfer(Pipe pipe, Input input, Output output) throws IOException {
	...
	final ArraySchemas.BoolArray ARRAY_BOOL_DERIVED_SCHEMA = new ArraySchemas.BoolArray(this, null, false) {
		@Override
	...
	final ArraySchemas.CharArray ARRAY_CHAR_DERIVED_SCHEMA = new ArraySchemas.CharArray(this, null, false) {
		@Override
		public Object readFrom(Input input, Object owner) throws IOException {
			if (ArraySchemas.ID_ARRAY_LEN != input.readFieldNumber(this))
				throw new ProtostuffException("Corrupt input.");
	...
	final ArraySchemas.ShortArray ARRAY_SHORT_DERIVED_SCHEMA = new ArraySchemas.ShortArray(this, null, false) {
		@Override
		public Object readFrom(Input input, Object owner) throws IOException {
			if (ArraySchemas.ID_ARRAY_LEN != input.readFieldNumber(this))
				throw new ProtostuffException("Corrupt input.");
	...
	final ArraySchemas.Int32Array ARRAY_INT32_DERIVED_SCHEMA = new ArraySchemas.Int32Array(this, null, false) {
		@Override
		public Object readFrom(Input input, Object owner) throws IOException {
			if (ArraySchemas.ID_ARRAY_LEN != input.readFieldNumber(this))
				throw new ProtostuffException("Corrupt input.");
	...
	final ArraySchemas.Int64Array ARRAY_INT64_DERIVED_SCHEMA = new ArraySchemas.Int64Array(this, null, false) {
		@Override
		public Object readFrom(Input input, Object owner) throws IOException {
			if (ArraySchemas.ID_ARRAY_LEN != input.readFieldNumber(this))
				throw new ProtostuffException("Corrupt input.");
	...
	final ArraySchemas.FloatArray ARRAY_FLOAT_DERIVED_SCHEMA = new ArraySchemas.FloatArray(this, null, false) {
		@Override
		public Object readFrom(Input input, Object owner) throws IOException {
			if (ArraySchemas.ID_ARRAY_LEN != input.readFieldNumber(this))
				throw new ProtostuffException("Corrupt input.");
	...
	final ArraySchemas.DoubleArray ARRAY_DOUBLE_DERIVED_SCHEMA = new ArraySchemas.DoubleArray(this, null, false) {
		@Override
		public Object readFrom(Input input, Object owner) throws IOException {
			if (ArraySchemas.ID_ARRAY_LEN != input.readFieldNumber(this))
				throw new ProtostuffException("Corrupt input.");
